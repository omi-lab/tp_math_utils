...
//float grid3d::get_grid_value(unsigned int i, unsigned int j, unsigned int k) {
//	if (F && i <= N[0] && j <= N[1] && k <= N[2])
//		return F[k][j][x_data > 1? i*x_data: i];
//  return std::numeric_limits<float>::quiet_NaN();
//}

//float grid3d::interpolated_value(double x, double y, double z) {
...
//float grid3d::bad_value(double*) const {
//  return std::numeric_limits<float>::quiet_NaN();
//}
...
//float grid3d::trilinear(double *r) const {
//	unsigned int i[3];
//	double t[3], s[3];
//	for (int j = 0; j < 3; j++) {
//		if ((periodic>>j)&0x01) {
//      t[j] = (r[j] - double(L[j])*std::floor(r[j]/double(L[j])))/d[j];
//      i[j] = int(t[j]);
//			t[j] -= i[j];
//			i[j] = (i[j]? i[j] - 1: N[j] - 1);
//		} else {
...
//		s[j] = 1.0 - t[j];
//	}
//  return s[0]*s[1]*s[2]*double(F[i[2]][i[1]][i[0]]) + t[0]*s[1]*s[2]*double(F[i[2]][i[1]][i[0] + 1]) + s[0]*t[1]*s[2]*double(F[i[2]][i[1] + 1][i[0]]) + s[0]*s[1]*t[2]*double(F[i[2] + 1][i[1]][i[0]]) + t[0]*t[1]*s[2]*double(F[i[2]][i[1] + 1][i[0] + 1]) + s[0]*t[1]*t[2]*double(F[i[2] + 1][i[1] + 1][i[0]]) + t[0]*s[1]*t[2]*double(F[i[2] + 1][i[1]][i[0] + 1]) + t[0]*t[1]*t[2]*double(F[i[2] + 1][i[1] + 1][i[0] + 1]);
//}

////from https://facyt-quimicomp.neocities.org/Vega_en.html#c_library
//float grid3d::tricubic(double *r) const {
...
//		if ((periodic>>i)&0x01)
//		{
//      t = (r[i] - double(L[i])*floor(r[i]/double(L[i])))/d[i];
//      iR[i] = int(t);
//			t -= iR[i];
...
//			for (i = 0; i < 4; i++)
//			{
//        f += w[0][i]*w[1][j]*w[2][k]*double(F[z][y][x]);
//				if ((++x) > N[0]) x = 1;
//			}
//			if ((++y) > N[1]) y = 1;
//		}
...
//		nonortho = 1;
//    double ca = cos(double(Ang[0])*(M_PI/180.0));
//    double cb = cos(double(Ang[1])*(M_PI/180.0));
//    double aux1 = double(Ang[2])*(M_PI/180.0);
//		double sg = sin(aux1);
//		double cg = cos(aux1);
//		aux1 = ca - cb*cg;
...
//void grid3d::set_grid_value(unsigned int i, unsigned int j, unsigned int k, float value) {
...
//			else
//        memcpy(F[k][j], G.F[k][j], (N[0] + 1)*sizeof(float));
//		}
//}
...
//int grid3d::alloc_F() {
//	unsigned int j, k;
//  F = new (nothrow) float**[N[2] + 1];
//	if (!F)
//		return -1;
//	for (k = 0; k <= N[2]; ++k) {
//    F[k] = new (nothrow) float*[N[1] + 1];
//		if (!F[k])
//			return -1;
//		for (j = 0; j <= N[1]; ++j) {
//      F[k][j] = new (nothrow) float[N[0] + 1];
//			if (!F[k][j]) {
//				while (j)
//					delete[] F[k][--j];
//				delete[] F[k];
...
//int grid3d::set_data_pointer(unsigned int Nx, unsigned int Ny, unsigned int Nz, float* data) {
//	free_F();
...
//	N[1] = Ny - 1;
//	N[2] = Nz - 1;
//  F = new (nothrow) float**[Nz];
//	if (!F)
//		return -2;
//	for (unsigned int k = 0; k < Nz; ++k) {
//    F[k] = new (nothrow) float*[Ny];
//		if (!F[k]) {
//			while (k)
//				delete[] F[--k];
//			delete[] F;
...
//	G->N[1] = Nj - 1;
//	G->N[2] = Nk - 1;
//  G->F = new (nothrow) float**[Nk];
//	if (!G->F) {
//		delete G;
//		return -2;
//	}
//	for (unsigned int k = 0; k < Nk; ++k) {
//    G->F[k] = new (nothrow) float*[Nj];
//		if (!G->F[k]) {
//			while (++k < Nk)
//				G->F[k] = 0;
//			delete G;
...
//		}
//		if (!((++k)&63)) {
//      float ***pt = new (nothrow) float**[k + 64];
//			if (!pt) {
//				m = -2;
//				break;
//			}
...
//			F = pt;
//		}
//    F[k] = new (nothrow) float*[res];
//		if (!F[k]) {
//			m = -2;
//			break;
//		}
//		for (j = 0; j != res; ++j)
//      F[k][j] = new (nothrow) float[res];
//		if (!F[k][N[1]]) {
//			m = -2;
//			break;
//		}
...
//					if (byte == -2)
//            F[k][j][i] = float((ui>>8)|(ui<<8));
//					else if (byte == -4)
//            F[k][j][i] = float((ui>>24)|((ui>>8)&0x00f0)|((ui<<8)&0x0f00)|(ui<<24));
//					else
//            F[k][j][i] = float((ui>>16)|(ui&0x00f0)|(ui<<16));
//				}
//	} else {
//		for (k = 0; k != n[2]; ++k)
//			for (j = 0; j != n[1]; ++j)
...
//			if (x_data > 1)
//				for (unsigned int i = 0; i <= N[0]; ++i)
//          out.write(reinterpret_cast<char*>(F[k][j] + i*x_data), sizeof(float));
//			else
//        out.write(reinterpret_cast<char*>(F[k][j]), (N[0] + 1)*sizeof(float));
//		}
//	return (out.good()? 0: -4);
//}
